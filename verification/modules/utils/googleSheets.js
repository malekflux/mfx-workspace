...
async function fetchGoogleSheetData(sheetId, apiKey) {
  ...
  return (data.values || []).filter((r) => r.some(Boolean)).map((r) => ({ refId: r[0]?.trim() || "", businessName: r[1]?.trim() || "", contactPerson: r[2] || "", phone: r[3] || "", services: r[4] || "", billingModel: r[5] || "", monthlyRate: Number((r[6] || "").replace(/[^\d.-]/g, "")), paymentMethod: r[7] || "", paymentDueDay: Number(r[8] || 1), dueMethod: r[9] || "", contractStartDate: r[10] || "", paymentStatus: r[11] || "", status: r[12] || "", driveLink: r[13] || "", accountPIC: r[14] || "", notes: r[15] || "", paidAmount: r[16]?.trim() ? Number(r[16].replace(/[^\d.-]/g, "")) : void 0 }));
}
function parseSheetRowToProject(row, clientId) {
  ...
  if (status.includes("partial") && row.paidAmount === void 0) throw new Error(`${row.refId}: partial payment needs the actual amount in column Q. No 50% assumption is made.`);
