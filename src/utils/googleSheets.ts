...
import { normalizeDueDays } from './projectTerms';
...
export function parseSheetRowToProject(row:SheetRow,clientId:string):Project {
 ...
 const billingModel=billing.includes('fixed')?'fixed':billing.includes('hourly')?'hourly':billing.includes('retainer')?'retainer':'monthly';
 const paymentDueDays=normalizeDueDays(row.dueMethod || [row.paymentDueDay]);
 const paymentDueMethod=billingModel==='hourly'?undefined:row.dueMethod.includes('50')?'split-50-50':'due-days';
 const totalCents=Math.round(row.monthlyRate*100);
 const share=Math.floor(totalCents/names.length);
 return {id:`sheet_project_${row.refId}`,clientId,refId:row.refId,sourceRef:row.refId,services:names.map((name,i)=>({id:`${row.refId}_${i}`,name,basePrice:(share+(i<totalCents%names.length?1:0))/100})),billingModel,totalAmount:row.monthlyRate,paidAmount:paid,remainingAmount:Math.round((row.monthlyRate-paid)*100)/100,paymentMethod:row.paymentMethod.toLowerCase().includes('instapay')?'instapay':row.paymentMethod.toLowerCase().includes('bank')?'bank-transfer':row.paymentMethod.toLowerCase().includes('card')?'card':'cash',paymentDueMethod,paymentDueDays,paymentDueDay:paymentDueDays[0]||row.paymentDueDay,dueMethod:row.dueMethod,contractStartDate:row.contractStartDate,paymentStatus:paid===row.monthlyRate?'paid':paid>0?'partial':status==='overdue'?'overdue':'pending',projectStatus:row.status.toLowerCase().includes('complete')?'completed':row.status.toLowerCase().includes('pause')?'paused':row.status.toLowerCase().includes('cancel')?'cancelled':'active',driveLink:/^https:\/\//i.test(row.driveLink)?row.driveLink:undefined,accountPIC:row.accountPIC.split(',').map(value=>value.trim()).filter(Boolean),notes:row.notes,currency:'EGP',currencySymbol:'EGP'};
}
